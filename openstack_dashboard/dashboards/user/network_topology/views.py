...
from openstack_dashboard.dashboards.user.network_topology.ports \
    import tables as ports_tables
from openstack_dashboard.dashboards.user.network_topology.routers \
    import tables as routers_tables

from openstack_dashboard.dashboards.user.networks import\
    views as n_views
from openstack_dashboard.dashboards.user.networks import\
    workflows as n_workflows
from openstack_dashboard.dashboards.user.routers import\
    views as r_views


class NTCreateRouterView(r_views.CreateView):
    template_name = 'user/network_topology/create_router.html'
    success_url = reverse_lazy("horizon:user:network_topology:index")
    page_title = _("Create a Router")


class NTCreateNetwork(n_workflows.CreateNetwork):
    def get_success_url(self):
        return reverse("horizon:user:network_topology:index")
...
class RouterView(r_views.IndexView):
    ...
    template_name = 'user/network_topology/iframe.html'
...
class NetworkTopologyView(views.HorizonTemplateView):
    template_name = 'user/network_topology/index.html'
...
class JSONView(View):
    ...
    def _get_networks(self, request):
        ...
        self.add_resource_url('horizon:user:networks:detail',
                              networks)
    ...
    def _get_routers(self, request):
        ...
        self.add_resource_url('horizon:user:routers:detail', routers)
        return routers
    ...
    def _get_ports(self, request):
        ...
        self.add_resource_url('horizon:user:networks:ports:detail',
                              ports)
        return ports
