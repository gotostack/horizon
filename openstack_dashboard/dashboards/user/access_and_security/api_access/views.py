...
def download_ec2_bundle(request):
    ...
    template = 'user/access_and_security/api_access/ec2rc.sh.template'
    try:
...
def download_rc_file(request):
    template = 'user/access_and_security/api_access/openrc.sh.template'
    try:
...
class CredentialsView(forms.ModalFormMixin, views.HorizonTemplateView):
    template_name = 'user/access_and_security/api_access/credentials.html'
