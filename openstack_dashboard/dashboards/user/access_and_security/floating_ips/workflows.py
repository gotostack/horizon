...
ALLOCATE_URL = "horizon:user:access_and_security:floating_ips:allocate"
...
class AssociateIPAction(workflows.Action):
    ...
    def get_targes(self):
        networkclient = api.network.NetworkClient(self.request)
        tenant_id = self.request.user.tenant_id
        ports = api.neutron.port_list(self.request, tenant_id=tenant_id)

        reachable_subnets = networkclient.floating_ips._get_reachable_subnets(
            ports)

        vip_dict = dict(
            [(v['vip_port_id'], v['name'])
                for v in api.lbaas_v2.loadbalancer_list(self.request,
                                                        tenant_id=tenant_id)])
        targets = []
        for p in ports:
            ...
            if p.device_owner.startswith('network:'):
                continue
            port_id = p.id
            server_name = vip_dict.get(port_id)
            ...
            for ip in p.fixed_ips:
                ...
                targets.append(api.neutron.FloatingIpTarget(target))
        return targets
    ...
    def _get_target_list(self):
        targets = []
        try:
            targets = self.get_targes()
        except Exception:
            redirect = reverse('horizon:user:access_and_security:index')
            exceptions.handle(self.request,
                              _('Unable to retrieve port list.'),
                              redirect=redirect)
        return targets
...
class IPAssociationWorkflow(workflows.Workflow):
    ...
    success_url = "horizon:user:access_and_security:index"
